...
def training(
    ...
    if wl2_epochs > 0:
        wl2_model = models.Model(
            unet_model.inputs, [unet_model.get_layer("unet2_likelihood").output]
        )
...
class BrainGeneratorGroup(BrainGenerator):
    def __init__(self, grouping_labels=None, **kwargs):
        ...
        ) = self._build_labels_to_image_model_group()

    def _build_labels_to_image_model_group(self):
        # build_model
        lab_to_im_model = labels_to_image_model_group(
...
def labels_to_image_model_group(
    labels_shape,
    n_channels,
    generation_labels,
    output_labels,
    ...
    grouped_labels = layers.ConvertLabels(
        generation_labels, dest_values=grouping_labels
    )(labels)
    grouped_labels = layers.RandomSpatialDeformation(
        scaling_bounds=0.05,
        rotation_bounds=3,
    ...
    grouped_labels = KL.Lambda(
        lambda x: tf.one_hot(tf.cast(x[..., 0], "int32"), n_group, axis=-1)
    )(grouped_labels)
    split = KL.Lambda(lambda x: tf.split(x, [1] * n_group, axis=-1))(grouped_labels)
    channels = [
        layers.RandomDilationErosion(
            1, 1, 5, 0.9, operation="random", return_mask=True
        )(c)
        for c in split
    ]
    ...
    if flipping:
        ...
        labels, grouped_labels = layers.RandomFlip(
    ...
    if crop_shape != output_shape:
        ...
        grouped_labels = l2i_et.resample_tensor(
            grouped_labels, output_shape, interp_method="nearest"
        )
    ...
    image = KL.Lambda(
        lambda x: tf.concat([x[0], tf.cast(x[1], dtype=x[0].dtype)], axis=-1),
        name="image_out",
    )([image, grouped_labels, labels])
