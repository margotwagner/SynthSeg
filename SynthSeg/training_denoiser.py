...
def training(
    ...
    if target_segmentation_labels is None:
        ...
        target_label_list, _ = utils.get_list_labels(
            label_list=target_segmentation_labels
        )
    n_labels = np.size(target_label_list)

    # create augmentation model
    labels_shape, _, _, _, _, _ = utils.get_volume_info(
        list_paths_input_labels[0], aff_ref=np.eye(4)
    )
    ...
    model_inputs = build_model_inputs(
        path_inputs=list_paths_input_labels,
    ...
    if wl2_epochs > 0:
        wl2_model = models.Model(
            l2l_model.inputs, [l2l_model.get_layer("l2l_likelihood").output]
        )
...
def build_augmentation_model(
    ...
    # define model inputs
    net_input = KL.Input(
        shape=labels_shape + [1], name="l2l_noisy_labels_input", dtype="int32"
    )
    target_input = KL.Input(
        shape=labels_shape + [1], name="l2l_target_input", dtype="int32"
    )

    # deform labels
    noisy_labels, target = layers.RandomSpatialDeformation(
        scaling_bounds=scaling_bounds,
        rotation_bounds=rotation_bounds,
        shearing_bounds=shearing_bounds,
    ...
    if prob_erosion_dilation > 0:
        noisy_labels = layers.RandomDilationErosion(
            min_erosion_dilation, max_erosion_dilation, prob=prob_erosion_dilation
        )(noisy_labels)
    ...
    noisy_labels = KL.Lambda(
        lambda x: tf.one_hot(x[0][..., 0], depth=n_labels), name="noisy_labels_out"
    )([noisy_labels, target])

    # build model and return
    brain_model = models.Model(
        inputs=[net_input, target_input], outputs=[noisy_labels, target]
    )
    return brain_model
