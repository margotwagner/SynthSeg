...
def predict(
    ...
    (
        ...
        path_predictions, path_corrections, path_posteriors, path_volumes, recompute
    )

    # get label list
    input_segmentation_labels = utils.get_list_labels(
        label_list=input_segmentation_labels
    )[0]
    input_segmentation_labels, unique_idx = np.unique(
        input_segmentation_labels, return_index=True
    )
    if target_segmentation_labels is not None:
        target_segmentation_labels = utils.get_list_labels(
            label_list=target_segmentation_labels
        )[0]
        target_segmentation_labels, unique_idx = np.unique(
            target_segmentation_labels, return_index=True
        )
    else:
    ...
    if unique_vol_file & (path_volumes[0] is not None):
        write_csv(
            path_volumes[0], None, True, target_segmentation_labels, names_segmentation
        )
    ...
    for i in range(len(path_predictions)):
        ...
        if compute[i]:
            # preprocessing
            prediction, aff, h, im_res, shape, pad_idx, crop_idx = preprocess(
                path_prediction=path_predictions[i],
                n_levels=n_levels,
                crop=cropping,
                min_pad=min_pad,
            )
            ...
            if path_volumes[i] is not None:
                row = [os.path.basename(path_predictions[i]).replace(".nii.gz", "")] + [
    ...
    if gt_folder is not None:
        ...
        if compute_distances:
            ...
            path_hausdorff = (
                path_hausdorff_99
            ) = path_hausdorff_95 = path_mean_distance = None
...
def prepare_output_files(
    path_predictions, out_corrections, out_posteriors, out_volumes, recompute
):
    # check inputs
    ...
    if basename[-4:] == ".txt":
        ...
        if not os.path.isfile(path_predictions):
            raise Exception(
                "provided text file containing paths of input prediction does not exist"
                % path_predictions
            )
        ...
        def text_helper(path, name):
            if path is not None:
                assert path[-4:] == ".txt", (
                    "if path_predictions given as text file, so must be %s" % name
                )
        ...
        out_corrections, recompute_cor, _ = text_helper(
            out_corrections, "path_corrections"
        )
    ...
    # path_predictions is a folder
        ...
        def helper_dir(path, name, file_type, suffix):
            ...
            if path is not None:
                assert path[-4:] != ".txt", (
                    "%s can only be given as text file when path_predictions is." % name
                )
                if file_type == "csv":
                    if path[-4:] != ".csv":
                        print(
                    ...
                    path = [
                        os.path.join(path, os.path.basename(p))
        ...
        # use helper on all outputs
        out_corrections, recompute_cor, _ = helper_dir(
            out_corrections, "path_corrections", "", "synthseg"
        )
    ...
    # path_predictions is an image
        ...
        # input prediction
        assert os.path.isfile(path_predictions), (
            ...
            if path is not None:
                ...
                if file_type == "csv":
                    ...
                    if (
                        ...
                        file_name = os.path.basename(path_predictions[0]).replace(
                            ".nii", "_%s.nii" % suffix
                        )
        ...
        # use helper on all outputs
        out_corrections, recompute_cor, _ = helper_im(
            out_corrections, "path_corrections", "", "synthseg"
        )
    ...
    recompute_list = [
        recompute | re_cor | re_post | re_vol
        for (re_cor, re_post, re_vol) in zip(
            recompute_cor, recompute_post, recompute_volume
        )
    ]
...
def preprocess(path_prediction, n_levels, crop=None, min_pad=None):
    ...
    pred, _, aff_pred, n_dims, _, h_pred, res_pred = utils.get_volume_info(
        path_prediction, True
    )

    # align image
    pred = edit_volumes.align_volume_to_ref(
        pred, aff_pred, aff_ref=np.eye(4), n_dims=n_dims
    )
    ...
    if crop is not None:
        ...
        pred, crop_idx = edit_volumes.crop_volume(
            pred, cropping_shape=input_shape, return_crop_idx=True
        )
    else:
        crop_idx = None
        input_shape = [
    ...
    pred, pad_idx = edit_volumes.pad_volume(
        pred, padding_shape=input_shape, return_pad_idx=True
    )

    # add batch and channel axes
    pred = utils.add_axis(
        pred, axis=0
    )  # channel axis will be added later when computing one-hot
...
def build_model(
    ...
    labels = KL.Lambda(
        lambda x: tf.one_hot(
            tf.cast(x, dtype="int32"), depth=len(input_label_list), axis=-1
        )
    )(labels)
