...
def validate_training(
    ...
    # loop over models
    list_models = utils.list_files(models_dir, expr=["qc", ".h5"], cond_type="and")[
        ::step_eval
    ]
    ...
    for model_idx, path_model in enumerate(list_models):
        ...
        if (not os.path.isfile(score_path)) | recompute:
            ...
            predict(
                path_predictions=prediction_dir,
...
def plot_validation_curves(
    ...
    for idx, (
        ...
        for epoch_dir in list_epochs_dir:
            # build names and create folders
            path_epoch_scores = utils.list_files(
                os.path.join(net_val_dir, epoch_dir), expr="diff"
            )
            if len(path_epoch_scores) == 1:
                ...
                if eval_idx is not None:
                    ...
                    if skip_first_dice_row:
                        list_net_scores.append(
                            np.mean(np.abs(np.load(path_epoch_scores)[1:, :]))
                        )
                    else:
                        list_net_scores.append(
                            np.mean(np.abs(np.load(path_epoch_scores)))
                        )
