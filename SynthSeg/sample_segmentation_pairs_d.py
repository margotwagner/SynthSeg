...
def sample_segmentation_pairs(
    ...
    assert len(path_images) == len(
        path_labels
    ), "There should be as many images as label maps."
    ...
    segmentation_labels, _ = utils.get_list_labels(
        label_list=segmentation_labels, labels_dir=labels_dir
    )
    n_labels = np.size(np.unique(segmentation_labels))

    # create augmentation model
    im_shape, _, n_dims, n_channels, _, atlas_res = utils.get_volume_info(
        path_images[0], aff_ref=np.eye(4)
    )
    ...
    generation_model = models.Model(
        inputs=unet_model.inputs, outputs=list_output_tensors
    )
    ...
    while i <= n_examples:
        ...
        # save results
        for output, name, res_dir in zip(
            outputs,
            ...
            for b in range(batchsize):
                ...
                path = os.path.join(
                    res_dir, tmp_name + "_%.{}d".format(n) % i + ".nii.gz"
                )
                if batchsize > 1:
...
def build_augmentation_model(
    ...
    # apply bias field
    if bias_field_std > 0:
        image = layers.BiasFieldCorruption(bias_field_std, bias_shape_factor, False)(
            image
        )

    # intensity augmentation
    image = layers.IntensityAugmentation(
        noise_std, gamma_std=gamma_std, contrast_inversion=True
    )(image)
    ...
    for i, channel in enumerate(split):
        ...
        min_res = np.array(
            utils.reformat_to_list(min_res, length=n_dims, dtype="float")
        )
        ...
        resolution, blur_res = layers.SampleResolution(
            min_res, max_res_iso, max_res_aniso
        )(channel)
        ...
        channel = layers.MimicAcquisition(
            atlas_res, min_res, im_shape, False, noise_std_lr
        )([channel, resolution])
    ...
    brain_model = models.Model(
        inputs=[image_input, labels_input], outputs=[image, labels]
    )
