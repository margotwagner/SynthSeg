...
def predict(
    ...
    for i in range(len(path_images)):
        ...
        if compute[i]:
            # preprocessing
            image, mask, aff, h, im_res, shape, pad_idx, crop_idx = preprocess(
...
def prepare_output_files(
    path_images, path_masks, out_seg, out_posteriors, out_volumes, recompute
):
    # check inputs
    ...
    # path_images is an image
        ...
        # masks
        assert os.path.isfile(path_masks), (
            "file does not exist: %s \n"
    ...
    recompute_list = [
        recompute | re_seg | re_post | re_vol
        for (re_seg, re_post, re_vol) in zip(
            recompute_seg, recompute_post, recompute_volume
        )
    ]
...
def preprocess(path_image, path_mask, n_levels, crop=None, min_pad=None):
    ...
    mask = edit_volumes.align_volume_to_ref(
        mask, aff, aff_ref=np.eye(4), n_dims=n_dims, return_copy=False
    )
...
def build_model(
    ...
    labels = KL.Lambda(
        lambda x: tf.one_hot(tf.cast(x, "int32"), depth=len(mask_labels), axis=-1)
    )(input_labels)
    image = KL.Lambda(lambda x: tf.cast(tf.concat(x, axis=-1), "float32"))(
        [input_image, labels]
    )
...
def postprocess(
    ...
    if strict_masking:
        ...
        post_patch[..., 0] = edit_volumes.mask_volume(
            post_patch[..., 0], mask=mask < 0.1, return_copy=False
        )
    ...
    seg_patch = edit_volumes.crop_volume_with_idx(
        seg_patch, pad_idx, n_dims=n_dims, return_copy=False
    )
    post_patch = edit_volumes.crop_volume_with_idx(
        post_patch, pad_idx, n_dims=n_dims, return_copy=False
    )
