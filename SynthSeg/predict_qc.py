...
def predict(
    ...
    gt_qc_results = (
        ...
        if path_gt_results is not None
        else None
    )
    ...
    for idx, (path_prediction, path_gt) in enumerate(zip(path_predictions, path_gts)):
        ...
        prediction, gt_scores = preprocess(
            path_prediction, path_gt, shape, labels_list, labels_to_convert, convert_gt
        )
...
def prepare_output_files(path_predictions, path_gts, path_qc_results):
    ...
    if path_qc_results[-4:] != ".npy":
        print(
            "Path for QC outputs provided without npy extension. Adding npy extension."
        )
        path_qc_results += ".npy"
    ...
    if path_gts is not None:
        ...
        assert len(path_gts) == len(
            path_predictions
        ), "not the same number of predictions and GTs"
...
def preprocess(
    ...
    gt_scores = (
        evaluate.fast_dice(pred, gt, np.unique(labels_list)) if gt is not None else None
    )
