...
def training(
    ...
    im_shape, _, _, n_channels, _, atlas_res = utils.get_volume_info(
...
def build_augmentation_model(
    ...
    crop_shape, output_shape = get_shapes(
        im_shape, output_shape, atlas_res, target_res, output_div_by_n
    )
    ...
    image = layers.IntensityAugmentation(
        6, clip=False, normalise=True, gamma_std=0.5, separate_channels=True
    )(image)
    ...
    if data_res is not None:
        ...
        if crop_shape != output_shape:
            labels = l2i_et.resample_tensor(
                labels, output_shape, interp_method="nearest"
            )
...
def build_model_inputs(
    ...
    while True:
        ...
        indices = npr.choice(
            np.arange(len(path_outputs)), size=batchsize, p=subjects_prob
        )
        ...
        for idx in indices:
            # get a batch input
            batch_input = utils.load_volume(
                path_inputs[idx], aff_ref=np.eye(4), dtype=dtype_input
            )
            if n_channels > 1:
            ...
            # get a batch output
            batch_output = utils.load_volume(
                path_outputs[idx], aff_ref=np.eye(4), dtype=dtype_output
            )
