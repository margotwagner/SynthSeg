...
class fMRI_Segmentation:
    ...
    def segment_fmri(self):
        ...
        with ProcessPoolExecutor() as executor:
            for i, data in enumerate(self.raw_fmri):
                futures.append(executor.submit(self._segment_subject, i, data))
    ...
    def _segment_subject(self, i, data):
        """
        This function segments an individual subject’s fMRI data.
        It has been extracted for parallel processing.
        """
        run = data.split("_")[-2]
        ...
        try:
            ...
            filtered_output_path = os.path.join(self.saving_path, subject, f"filt_fMRI_segmented_{subject}_{run}.csv")
            if not os.path.exists(filtered_output_path):
                try:
                    ...
                    error_log.append(f"Filt error: {subject}")

        except Exception as e:
            print(f"Error segmenting {subject}: {e}")
            error_log.append(f"Segmentation error: {subject}")

        return error_log #f"Segmentation error: {subject}"

    def band_pass_filt_fmri(self):
        futures = []
        with ProcessPoolExecutor() as executor:
            for data_path in glob.glob(os.path.join(self.saving_path, "**", "fMRI_segmented_*.csv"), recursive=True):
                futures.append(executor.submit(self._filter_subject_data, data_path))

            for future in as_completed(futures):
                future.result()  # Collect and handle errors if needed

    def _filter_subject_data(self, data_path):
        """
        Applies the band-pass filter to the fMRI data for an individual subject.
